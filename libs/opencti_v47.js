...
/***/ (function(module, exports, __webpack_require__) {
	...
	var hvsApi = __webpack_require__(6);
	var subscriptionManager = __webpack_require__(7);
	...
	module.exports = {
	    ...
	    */
	     ...
	     * @param {Function} args.callback - {Function} callback - Executes function on success and returns an error on failure.
	     */
	    ...
	    /**************************************************************************************************
	     * Dev Preview API methods
	     **************************************************************************************************/
	    /**
	     * Registers a function to be invoked when the specified MessageChannel receives data
	     * @param {Object} args - An object holding arguments for calls to this method. 
	     * @param {string} args.channelName - The name of the MessageChannel
	     * @param {Function} args.listener - Function which will be called when the specified MessageChannel receives data
	     * @return {Object} subscription - A Subscription object which is the key to the subscribed MessageChannel name and function
	     */
	    subscribe: function subscribe(args) {
	        utils.validateArguments(arguments);
	        utils.validateListener(args);
	        utils.validateChannelName(args);
	        var userCallback = args ? args.callback : undefined;

	        var subscription = subscriptionManager.addSubscription(args.channelName, args.listener);

	        var callback = function callback(result) {
	            if (result.success) {
	                result.subscription = subscription;
	            } else {
	                listenerManager.unregisterListener('subscribe', args.channelName, args.listener);
	                subscriptionManager.getAndRemoveSubscription(subscription);
	            }
	            if (userCallback) {
	                userCallback(result);
	            }
	        ...
	        utils.invokeApiCall('subscribe', {
	            eventName: args.channelName,
	            subscription: subscription
	        }, callback, args.listener);
	    },

	    /**
	     * Unregisters a function to be invoked when the specified MessageChannel receives data
	     * @param {Object} args - An object holding arguments for calls to this method. 
	     * @param {Object} args.subscription - The Subscription object returned from the Subscribe method
	     */
	    unsubscribe: function unsubscribe(args) {
	        utils.validateArguments(arguments);
	        utils.validateSubscription(args);
	        var callback = args ? args.callback : undefined;

	        var subscription = subscriptionManager.getAndRemoveSubscription(args.subscription);
	        if (subscription) {
	            listenerManager.unregisterListener('subscribe', subscription.eventName, subscription.listener);
	            if (!listenerManager.getListenerCount('subscribe', subscription.eventName)) {
	                utils.invokeApiCall('unsubscribe', {
	    ...
	    /**
	     * Passes data to a specified MessageChannel
	     * @param {Object} args - An object holding arguments for calls to this method.  
	     * @param {string} args.channelName - Name of the MessageChannel to which to send the message
	     * @param {Object} args.message - Optional, serializable object to be sent to subscribers.
	     */
	    publish: function publish(args) {
	        ...
	        var message = utils.serializeAndValidateMessage(args);
	        utils.invokeApiCall('publish', {
	            eventName: args.channelName,
	            message: message
	        }, callback);
	    }
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var processPostMessage = function processPostMessage(event) {
	    ...
	    if (eventData && eventData.apiVersion === config.API_VERSION && eventData.apiType === config.API_TYPE && event.origin === config.sfdcIframeOrigin) {
	        ...
	        if (eventData.shouldExecuteCallback === true) {
	            callbackManager.executeCallback(eventData.methodName, eventData.callId, eventData.response);
	        }
	        if (eventData.shouldExecuteListener === true) {
	            listenerManager.executeListeners(eventData.methodName, eventData.eventName, eventData.response);
	        }
	    }
	};
	 ...
	 * @param  {Function} callback - The callback to be called after the API returns a value (success or failure).
	...
	var invokeApiCall = function invokeApiCall(methodName, args, callback, listener) {
	    ...
	    if (typeof listener === 'function') {
	        listenerManager.registerListener(params.methodName, args.eventName, listener);
	...
	/**
	 * Validates the eventName argument in args.
	 */
	var validateEventName = function validateEventName(args) {
	    if (!(args && args.eventName && typeof args.eventName === 'string')) {
	        throw new Error('The argument `eventName` must be a non-empty string!');
	    }
	};

	/**
	 * Validates the channelName argument in args.
	 */
	var validateChannelName = function validateChannelName(args) {
	    if (!(args && args.channelName && typeof args.channelName === 'string')) {
	        throw new Error('The argument `channelName` must be a non-empty string!');
	    }
	};

	/**
	 * Validates the subscription argument in args.
	 */
	var validateSubscription = function validateSubscription(args) {
	    if (!(args && args.subscription && _typeof(args.subscription) === 'object')) {
	        throw new Error('The argument `subscription` must be an object!');
	    }
	};

	/**
	 * Validates the message argument in args.
	 * @return {Object} _message - The message after it has been serialized and deserialized
	 */
	var serializeAndValidateMessage = function serializeAndValidateMessage(args) {
	    if (!args) {
	        throw new Error('Missing arguments!');
	    }
	    if (args.message) {
	        try {
	            var _message = JSON.parse(JSON.stringify(args.message));
	        } catch (error) {
	            throw new Error(error.message);
	        }
	        if ((typeof _message === 'undefined' ? 'undefined' : _typeof(_message)) !== 'object') {
	            throw new Error('The argument `message` must be a serializable object or null!');
	        }
	        return _message;
	    } else {
	        return undefined;
	...
	/**
	 * Validates the payload argument in args.
	 */
	var validatePayload = function validatePayload(args) {
	    if (!(args && typeof args.payload === 'string')) {
	        throw new Error('The argument `payload` must be a string!');
	    }
	};
	...
	module.exports = {
	    ...
	    serializeAndValidateMessage: serializeAndValidateMessage,
...
/***/ (function(module, exports) {
	...
	module.exports = {
	     ...
	     * @param {string} methodName - Represents the name of the API method being invoked.
	     * @param {string} eventName - The name of the event to which the listener is bound.
	     * @param {Function} listener - The function to be called whenever the API "methodName" returns a value.
	     */
	    registerListener: function registerListener(methodName, eventName, listener) {
	        if (eventName) {
	            listenersMap[methodName] = listenersMap[methodName] || {};
	            listenersMap[methodName][eventName] = listenersMap[methodName][eventName] || [];
	            listenersMap[methodName][eventName].push(listener);
	        } else {
	    ...
	    /**
	     * Unregisters the listener for the API method.
	     * @param {string} methodName - Represents the name of the API method being invoked.
	     * @param {string} eventName - The name of the event to which the listener is bound.
	     * @param {Function} listener - The function to be unregistered.
	     */
	    unregisterListener: function unregisterListener(methodName, eventName, listener) {
	        var listeners;
	        var indexToRemove;

	        if (eventName) {
	            listenersMap[methodName] = listenersMap[methodName] || {};
	            listeners = listenersMap[methodName][eventName] || [];
	        } else {
	            listeners = listenersMap[methodName] || [];
	        }

	        indexToRemove = listeners.indexOf(listener);
	    ...
	    /**
	     * Fetches the number of listeners for the API method.
	     * @param {string} methodName - Represents the name of the API method being invoked.
	     * @param {string} eventName - The name of the event to get the listener count.
	     * @return {number} - The number of the listeners registered for the specified methodName and/or eventName.
	     */
	    getListenerCount: function getListenerCount(methodName, eventName) {
	        var listeners;
	     ...
	     */
	    executeListeners: function executeListeners(methodName, eventName, data) {
	        var listeners;

	        if (eventName) {
	            listeners = listenersMap[methodName] && listenersMap[methodName][eventName];
	        } else {
	            listeners = listenersMap[methodName];
	        }
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = {
	    /**
	     * Registers a function to call when a Cadence/MyList work starts.
	     * @param {Object} args - An object holding arguments for calls to this method.
	     * @param {Function} args.listener - Function which will be called when a Cadence/MyList work starts.
	     */
	    onWorkStart: function onWorkStart(args) {
	        utils.validateArguments(arguments);
	        utils.validateListener(args);
	        utils.invokeApiCall('onWorkStart', {}, undefined, args.listener);
	    },

	    /**
	     * Completes a Cadence/MyList work.
	     * @param {Object} args - An object holding arguments for calls to this method.
	     * @param {string} args.workId - The ID of the work to complete.
	     * @param {Object} args.attributes - A set of key/value pairs required for completing the specified work.
	     * @param {Function} args.callback - Function which will be called upon completion of this method's invocation.
	     */
	    completeWork: function completeWork(args) {
	        utils.validateArguments(arguments);
	        var callback = args ? args.callback : undefined;
	        utils.invokeApiCall('completeWork', { workId: args.workId, attributes: args.attributes }, callback);
	    }
	};
...
/***/ (function(module, exports) {
	...
	// Contains a list of subscriptions (key-> subscription, value -> {Event Name, Listener} )

	var subscriptionsMap = new Map();

	/**
	 * Get a new Subscription object
	 */
	var getSubscriptionKey = function getSubscriptionKey() {
	    return Object.freeze({});
	};

	module.exports = {
	    /**
	     * Associates the eventName and listener to a subscription
	     * @param {string} eventName - the name of the event to subscribe
	     * @param {Function} listener - the listener function to subscribe
	     * @return {Object} subscriptionKey - the key to the subscribed event name and listener function
	     */
	    addSubscription: function addSubscription(eventName, listener) {
	        var subscriptionKey = getSubscriptionKey();
	        subscriptionsMap.set(subscriptionKey, { eventName: eventName, listener: listener });
	        return subscriptionKey;
	    },

	    /**
	     * Removes and returns the eventName and listener associated with a subscription
	     * @param {string} subscriptionKey - the key to the subscribed eventName and listener function
	     * @return {Object} subscription - an object with the subscribed eventName and listener function
	     */
	    getAndRemoveSubscription: function getAndRemoveSubscription(subscriptionKey) {
	        var subscription = subscriptionsMap.get(subscriptionKey);
	        subscriptionsMap.delete(subscriptionKey);
	        return subscription;
	    }
	};
